...
int proxy_parse_ssl_hello_ech_opt(char **args, int cur_arg, struct proxy *curpx, const struct proxy *defpx,
				  const char *file, int line)
{
    ...
    /*
     * Grab the ech key pair file (or TODO: directory) name.
     * This is UNDOUBTEDLY badly done, but let's get something to 
     * sorta work and then see what's what.
     */
	struct tcpcheck_ruleset *rs = NULL;
	struct tcpcheck_rules *rules = &curpx->tcpcheck_rules;
	struct tcpcheck_rule *chk;
